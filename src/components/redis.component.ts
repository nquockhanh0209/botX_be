...
export default class RedisComponent {
  ...
  async hSet(
    ...
    return this.redis.hset(key, String(field), value);
  }
   ...
   * @returns Promise
   */
  async hDel(name: string, key: string | number): Promise<number> {
    return this.redis.hdel(name, String(key));
  }
   ...
   * @returns Promise
   */
  async delete(key: string | number): Promise<number> {
